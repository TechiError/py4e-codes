...
def run(item, page, flist):
    fl = flist[item["title"]]
    adata = page.locator('#body_container > ul > li:nth-child(2) > a').inner_text().strip()
    pos = page.locator('#body_container > ul > li:nth-child(2) > b:nth-child(3)').inner_text().strip()
    count = page.locator('#body_container > ul > li:nth-child(2) > b:nth-child(4)').inner_text().strip()
